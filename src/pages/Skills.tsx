  ...
  return (
    <div className="min-h-screen bg-background relative">
      ...
      <main className="container mx-auto px-4 pt-24 pb-12 relative z-10">
        ...
        <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-6 max-w-7xl mx-auto">
          {skillCategories.map((category, index) => {
            ...
            return (
              <div
                key={index}
                className="group bg-accent2/15 border border-accent2/25 backdrop-blur-sm rounded-2xl p-6 shadow-lg hover:shadow-2xl hover:border-accent2/50 hover:-translate-y-1 transition-all duration-300 animate-fade-up"
                style={{ animationDelay: `${index * 100}ms` }}
              >
                {/* Category Header */}
                <div className="flex items-center gap-3 mb-6">
                  <div className="p-3 bg-accent2/20 rounded-xl transition-all duration-300 group-hover:bg-accent2/35 group-hover:scale-110 group-hover:rotate-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {category.skills.map((skill, idx) => (
                    <span
                      key={idx}
                      className="px-3 py-1.5 bg-accent2/20 rounded-lg text-sm font-medium text-foreground transition-all duration-200 hover:bg-accent2/35 hover:scale-110 hover:-translate-y-0.5 hover:text-accent2 hover:shadow-[0_0_14px_rgba(59,130,246,0.55)] cursor-default"
                    >
                      {skill}
                    </span>
                  ))}
        ...
        <div className="mt-12 bg-accent2/15 border border-accent2/25 backdrop-blur-sm rounded-2xl p-8 animate-scale-in">
          ...
          <div className="flex flex-wrap justify-center gap-3">
            {[
              ...
              <span
                key={index}
                className="px-4 py-2 bg-accent2/20 border border-accent2/25 backdrop-blur rounded-full text-sm font-medium text-foreground transition-all duration-300 hover:bg-accent2 hover:text-accent-foreground hover:scale-110 hover:shadow-[0_0_14px_rgba(59,130,246,0.55)] cursor-default"
              >
                {tool}
              </span>
            ))}
